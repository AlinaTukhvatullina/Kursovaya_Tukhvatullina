...
void InitTukhvatullinaSCENEObjects() {
    shaderGBuffer=Shader("gBuffer.vs", "gBuffer.fs");
    shaderLighting= Shader("floor.vs", "floor.fs");
    ...
    SceneFloor = Quad(vec3(0, 0, 0), 100, 100, SceneFloorMtl, shaderGBuffer);
    SceneFloor.Rotate(-90, vec3(1, 0, 0));


    bench = Model("Content\\Objects\\Bench\\goya_bench3.obj", vec3(10.0f, 0, -23.0f), shaderGBuffer);
    tumb = Model("Content\\Objects\\tumb\\tumb.obj", vec3(7.8f, 0.0f, -10.4f), shaderGBuffer);
    sofa1 = Model("Content\\Objects\\Sofa1\\Santorini2.obj", vec3(-9.0f, 0.0f, -20.0f), shaderGBuffer);
    flower = Model("Content\\Objects\\alocasia\\flower2.obj", vec3(-9.93f, 0, -4.248f), shaderGBuffer);
    TV= Model("Content\\Objects\\TV\\MyTV2.obj", vec3(0, 2.9f, -6.0f), shaderGBuffer);
    table= Model("Content\\Objects\\Table\\Wood_Table.obj", vec3(-5.2f, 0, -20.0f), shaderGBuffer);
    dog= Model("Content\\Objects\\Dog\\dog.obj", vec3(-12.9f, 0, -10.92f), shaderGBuffer);
    lampModel = Model("Content\\Objects\\lamp\\lamp2.obj", vec3(-27.2f, 0, -23.7f), shaderGBuffer);
    Crunch= Model("Content\\Objects\\Crunch\\CrunchBox.obj", vec3(-4.4f, 3.6f, -19.2f), shaderGBuffer);
    weight=Model("Content\\Objects\\Weight\\weight.obj", vec3(-2.6f, 0, -27.9f), shaderGBuffer);

    bench.Scale(vec3(0.01f, 0.01f, 0.01f));
    ...
    for (unsigned int i = 0; i < NR_LIGHTS; i++)
    {
        ...
        float xPos = ((rand() % 100) / 100.0) * 20.0 - 3.0-20.0;
        float yPos = ((rand() % 100) / 100.0) * 10.0;
        float zPos = ((rand() % 100) / 100.0) * 20.0 - 3.0- 20.0;
    ...
    shaderLightBox=Shader("lightBox.vs", "lightBox.fs");
    lCube = LightCube(vec3(0), shaderLightBox);
...
void TukhvatullinaScene(glm::mat4 projection, glm::mat4 view) {
    ...
    // directional light
    shaderLighting.setVec3("dirLight.direction", -0.2f, -1.0f, -0.3f);
    shaderLighting.setVec3("dirLight.ambient", 0.1f, 0.1f, 0.1f);
    shaderLighting.setVec3("dirLight.diffuse", 0.6f, 0.6f, 0.6f);
    shaderLighting.setVec3("dirLight.specular", 0.2f, 0.2f, 0.2f);

    //pointLights
    shaderLighting.setVec3("pointLights[0].position", vec3(-27.2f, 9.0f, -23.7f));
    shaderLighting.setVec3("pointLights[0].ambient", 0.1f, 0.1f, 0.1f);
    shaderLighting.setVec3("pointLights[0].diffuse", 1.0f, 0.8157f, 0.1098f);
    shaderLighting.setVec3("pointLights[0].specular", 0.8f, 0.8f, 0.8f);
    shaderLighting.setFloat("pointLights[0].constant", 1.0f);
    shaderLighting.setFloat("pointLights[0].linear", 0.009f);
    shaderLighting.setFloat("pointLights[0].quadratic", 0.005f);
    for (unsigned int i = 1; i < lightPositions.size(); i++)
    {
        shaderLighting.setVec3("pointLights[" + std::to_string(i) + "].position", lightPositions[i]);
        shaderLighting.setVec3("pointLights[" + std::to_string(i) + "].ambient", lightColors[i]);
        shaderLighting.setVec3("pointLights[" + std::to_string(i) + "].diffuse", lightColors[i]);
        shaderLighting.setVec3("pointLights[" + std::to_string(i) + "].specular", vec3(0.8));
        //shaderLighting.setFloat("pointLights[" + std::to_string(i) + "].constant", 1.0f);
        // update attenuation parameters and calculate radius
        const float constant = 1.0; // note that we don't send this to the shader, we assume it is always 1.0 (in our case)
        const float linear = 0.7;
        const float quadratic = 1.8;
        shaderLighting.setFloat("pointLights[" + std::to_string(i) + "].linear", linear);
        shaderLighting.setFloat("pointLights[" + std::to_string(i) + "].quadratic", quadratic);
        // then calculate radius of light volume/sphere
        const float maxBrightness = std::fmaxf(std::fmaxf(lightColors[i].r, lightColors[i].g), lightColors[i].b);
        float radius = 0.1;
        shaderLighting.setFloat("pointLights[" + std::to_string(i) + "].constant", radius);
    }
    ...
    {
        lCube.UpdateModel();
        lCube.Trans(lightPositions[i]);
        lCube.ScaleModel(vec3(0.5));
...
//
//    //Triangle test(vec3(0, 0, 0), 1, 1);
//    //test.draw(Shader("vertex_shader_Gradient.glsl", "fragment_shader_Gradient.glsl"));
//
//    /*Quad testQuad(vec3(0, -0.5f, 0), 1, 1, Texture("Content\\textures\\brick1.jpg"), texShader, vec3(1,0,1));
//    testQuad.UseShader();*/
//
//    MainShader.Use();
//
//    glm::mat4 view(1.0f);//Матрица смещения
//    glm::mat4 projection(1.0f);//матрица перспективы
//
//
//
...
//    GLint projLoc = glGetUniformLocation(MainShader.Program, "projection");
//
//
//    
...
//
//    
//    //testCube.translate(vec3(0.0f, (GLfloat)translateSpeed, 0.0f));
//    //testCube.Rotate(/*(GLfloat)sinf(scale1)*/rotSpeed, vec3(0.7f, 1.0f, 1.0f));
//    
//    for (int i = 0; i < cubes.size(); i++) {
//        if(cubes[i].rotateble)
//        cubes[i].Rotate(rotSpeed, vec3(1.0f, 1.0f, 0.0f));
//        cubes[i].draw();
//    }
//
//
//    /*viewLoc = glGetUniformLocation(Plane.ShaderProgram(), "view");
//    projLoc = glGetUniformLocation(Plane.ShaderProgram(), "projection");*/
//    //glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view));
//    ////// Note: currently we set the projection matrix each frame, but since the projection matrix rarely changes it's often best practice to set it outside the main loop only once.
//    //glUniformMatrix4fv(projLoc, 1, GL_FALSE, glm::value_ptr(projection));
//    /*Plane.draw();*/
...
//bool LoadShaders(const GLchar* vert, const GLchar* frag) {
//    // Построение и компиляция шейдеров
//    // Тут будет вершинный
...
//        return false;
//    }
//    //удаляем уже ненужное (потому что мы уже записали все)
//
